...
public static class BlockLiquidContainerBase_SpillContents_Transpiler
{
    public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
    {
        ...
        for (int i = 0; i < codes.Count; i++)
        {
            ...
            if (codes[i].opcode == OpCodes.Ldc_R8 && Math.Abs((double)codes[i].operand - 10.0) < 1e-3f)
            {
    ...
    public static bool ExtraCheck(ItemSlot containerSlot)
    {
        ...
        var container = containerSlot.Itemstack?.Block as BlockLiquidContainerBase;
        if (container == null) return true;
        ItemStack contentStack = container.GetContent(containerSlot.Itemstack);
